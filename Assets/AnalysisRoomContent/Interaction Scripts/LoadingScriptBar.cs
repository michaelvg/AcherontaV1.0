...
public class LoadingScriptBar : MonoBehaviour {
	
	public GameObject myInstBar; 
	public BarAttributes_C MyScript; 
	...
	private void ProcessData(String JsonString)
	{
		...
		for(int i = 0; i<BpString["d"]["results"].Count; i++)
		{
			...
			GameObject ball = Instantiate(myInstBar, pos, Quaternion.identity) as GameObject;
			ball.name = "Ball" + i; 
			MyScript = ball.GetComponent<BarAttributes_C>();
